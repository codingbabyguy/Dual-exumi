...
class TactileCollectionEnv:
    ...
    def get_and_save_data(self):
        ...
        for camera_name, camera in self.camera_dict.items():
            ...
            if camera_data is None:
                ...
                if ts_count == 0 or frame_count == 0:
        ...
        for camera_name, camera_data in self.last_camera_data.items():
            save_summary["cameras"][camera_name] = self._save_video_and_json(camera_name, camera_data)

        if self.last_angle_data:
            save_summary["angle"] = self._save_angle_data(self.last_angle_data)
        else:
    ...
    def _save_video_and_json(self, camera_name, camera_data):
        if camera_data is None:
            ...
            return {
                "saved": False,
                "reason": "camera_data_none",
            }

        if camera_data.get("color", None) is None or camera_data.get("timestamp", None) is None:
            print(f"[SAVE][CAMERA][WARN] {camera_name}: invalid camera payload, skip save")
            return {
                "saved": False,
    ...
    def _save_angle_data(self, angle_data):
        ...
        if n > 0:
            ...
            return {
                ...
                "first_ts": float(timestamps[0]),
                "last_ts": float(timestamps[-1]),
                "angle_min": min(angle_vals),
                "angle_max": max(angle_vals),
            }
        else:
