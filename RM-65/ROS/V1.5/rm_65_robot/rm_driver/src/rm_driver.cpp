...
/***** ********************************END****************************************/
    ...
    while(ros::ok())
    {
	    ...
	    //ROS_INFO("all cJSON data is:%s", socket_buffer);
            buffer_cnt++;
                    ...
                    switch(res)
                    {
                        ...
                        case PLAN_STATE_TYPE:
                         ...
                         case ARM_CURRENT_STATE:
                             for(i=0;i<3;i++)
                             {
                                Arm_State.joint[i] = Arm_Current_State.joint[i]/1000;
                                Arm_State.Pose[i] = Arm_Current_State.Pose[i]/1000000;
                             }
                             for(i=3;i<6;i++)
                             {
                                Arm_State.joint[i] = Arm_Current_State.joint[i]/1000;
                                Arm_State.Pose[i] = Arm_Current_State.Pose[i]/1000;
                             }
